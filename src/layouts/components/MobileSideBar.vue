<template>
	<transition appear enter-active-class="animated fadeIn" leave-active-class="animated fadeOut" :duration="2000">
		<div v-show="LeftDrawerState" class="mobile-menu onboarding-text-primary">
			<transition
				appear
				...
				:duration="1000"
			>
				<div v-show="LeftDrawerState" class="sidebar-content onboarding-bg-primary">
					<div class="sidebar-header relative-position">
						...
						/>
						<div class="q-ml-md q-pa-lg text-white absolute-bottom overflow-hidden">
							<q-avatar size="80px" class="bg-hover-accent-0 q-mb-sm">
								<q-img
									v-if="LoggedinUser.photo"
									:src="`data:image/png;base64, ${LoggedinUser.photo}`"
								/>
								<!-- <q-img v-else :src="require(`src/core/assets/${!LoggedinUser.sex ? 'male' : 'female'}-avatar.svg`)" /> -->
							</q-avatar>
							<h5 class="q-ma-none ellipsis text-20">{{ LoggedinUser.full_name }}</h5>
