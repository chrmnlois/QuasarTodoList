<template>
	<div>
		<div class="bg-white q-pa-lg">
			<!-- *************************HEADER******************************** -->
			<h4 class="onboarding-bg-accent-1 text-white q-pa-sm">Headers</h4>
			...
			<hr class="q-my-md" />
			<!-- *************************TEXTS******************************** -->
			<h4 class="onboarding-bg-accent-1 text-white q-pa-sm">Texts</h4>
      ...
      <div class="text-16">Texts 16px</div> -->
			...
			<!-- ************************INPUTS******************************* -->
			<hr class="q-my-md" />
			<h4 class="onboarding-bg-accent-1 text-white q-pa-sm">Inputs</h4>
			<q-input
				dense
				borderless
				placeholder="Input"
			...
			<q-select
				class="onboarding-border-radius-10 onboarding-border-accent-0 onboarding-select-field standard"
				dense
				borderless
				v-model="selectModel"
				...
				:rules="[(val) => (val !== null && val !== '') || '']"
				hide-bottom-space
				:popup-content-class="DarkMode ? 'onboarding-option-style-dark' : 'onboarding-option-style-light'"
			...
			<div class="flex justify-start items-center">
				...
				<q-select
					...
					:options="pageOptions"
					class="
						onboarding-select-field onboarding-border-radius-50
						height-31
						min-width-90
						onboarding-border-accent-0
						text-center text-15
						q-ml-md
					"
			...
			<q-input
				...
				lazy-rules
				mask="####-##-##"
				class="onboarding-input-field standard onboarding-border-accent-0 onboarding-border-radius-10 q-mb-md"
			>
			...
			<div v-for="(keyResult, index) in keyResults" :key="index">
				<div
					class="
						onboarding-border-accent-0 onboarding-border-radius-15
						q-px-md q-py-lg q-mt-md
						standard-form-width
					"
				>
					<div class="field">
						<div class="flex justify-between items-center">
							<q-item-label class="label key_result text-weight-bold q-mb-sm">Key Results</q-item-label>
							<div v-if="keyResults.length > 1 && index !== 0">
								<q-btn
									...
									@click="route.params.id ? openDialog(index) : removeKeyResult(index)"
								/>
							</div>
						</div>
					...
					<div class="q-mt-md">
						<q-item-label class="label text-weight-bold q-mb-sm">Tag Tasks</q-item-label>
						<q-select
						...
						<q-item-label class="label text-weight-bold q-mb-sm q-mt-md">Milestones</q-item-label>
						<q-select
							class="
								onboarding-border-radius-10 onboarding-border-accent-0 onboarding-select-field
								standard
							...
							<template v-slot:before-options>
								<div class="q-px-md q-pt-md q-pb-sm">
									<q-input
										...
										@update:model-value="milestoneRef[0].filter(milestoneSearch)"
										placeholder="Search"
									>
										<template v-slot:prepend>
											<q-icon class="onboarding-text-accent-0 q-mr-sm" name="search" />
										</template>
									</q-input>
								</div>
							</template>
			...
			<!-- *************************BUTTONS******************************** -->
			<hr class="q-my-md" />
			<h4 class="onboarding-bg-accent-1 text-white q-pa-sm">Buttons</h4>
			...
			<q-btn
				...
				flat
				label="Capslock"
				class="onboarding-border-accent-0 onboarding-bg-hover-accent-0 q-px-xl"
			/>
			<div class="q-py-md"></div>
			<q-btn
				rounded
			...
			<q-btn
				...
				class="onboarding-button onboarding-border-accent-0 text-white onboarding-bg-accent-0 q-px-xl"
			/>

			<!-- **************************TABLE******************************* -->
			<hr class="q-my-md" />
			<h4 class="onboarding-bg-accent-1 text-white q-pa-sm">Tables</h4>
			<q-table :rows="rows" :columns="columns" row-key="name" class="onboarding-table" flat>
				<template v-slot:body-cell-action="props">
					<q-td :props="props">
						<div>
							<q-btn icon="more_vert" flat round style="color: #2b3347; text-decoration: none" />
			...
			<h4 class="onboarding-bg-accent-1 text-white q-pa-sm">Notification</h4>
			<q-btn class="onboarding-bg-accent-0 text-white" label="Show Notification" @click="showNotify()" />

			<!-- **************************Card******************************* -->
			<hr class="q-my-md q-mt-xl" />
			<h4 class="onboarding-bg-accent-1 text-white q-pa-sm">Card</h4>
			<div class="standard-form-width">
				<!-- Start to copy here -->
				<q-card class="onboarding-border-radius-12">
					<q-card-section>
						<h6 class="q-my-none onboarding-text-accent-0">Card title</h6>
						Lorem ipsum dolor sit amet consectetur, adipisicing elit. Amet, facere
					</q-card-section>
				</q-card>
				<div class="q-my-lg"></div>
				<!-- Start to copy here -->
				<q-card class="onboarding-border-radius-12 no-shadow onboarding-border-accent-0 q-pa-md q-mb-md">
					<div class="flex justify-between onboarding-text-accent-2">
						<div>Product</div>
			...
			<!-- **************************Graph******************************* -->
			<hr class="q-my-md q-mt-xl" />
			<h4 class="onboarding-bg-accent-1 text-white q-pa-sm">Graph</h4>
			...
			<div
				class="
					...
					onboarding-border-accent-0
					q-pa-md
					onboarding-border-radius-15
				"
			>
				<vue-highcharts type="chart" :options="dailyPerformance" />
			</div>

			<!-- **************************Menu******************************* -->
			<hr class="q-my-md q-mt-xl" />
			<h4 class="onboarding-bg-accent-1 text-white q-pa-sm">Menu w/ expansion</h4>
			<div class="standard-form-width">
				<!-- Start to copy here -->
				<q-expansion-item>
					<template v-slot:header>
						<q-item-section avatar class="margin-left-10">
							<q-icon name="iconfont icon-task-list-fill" class="list-icon" />
						</q-item-section>
						<q-item-section class="list-text pm-text-primary"> Settings </q-item-section>
					</template>
					<q-item clickable v-ripple :to="{ name: 'fund-type' }">
						<q-item-section avatar class="margin-left-10">
							<i class="empty"></i>
						</q-item-section>
						<q-item-section class="list-text pm-text-primary">Fund Types</q-item-section>
					</q-item>
				</q-expansion-item>
			</div>
		</div>
...
export default {
  ...
  setup() {
    ...
    // Graph
    let dailyPerformanceLoadingState = ref(false);
    let dailyPerformance = ref({
      chart: {
        type: "spline",
      },
      accessibility: {
