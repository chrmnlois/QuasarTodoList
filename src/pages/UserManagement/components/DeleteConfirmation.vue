	...
	<div class="user-delete width-300">
		<div class="full-width flex justify-center">
			<div class="onboarding-border-radius-10 bg-white width-auto q-pa-sm">
				<q-icon name="iconfont icon-delete-fill" class="onboarding-text-accent-0" size="30px" />
			</div>
		</div>
		...
		<q-input
			...
			class="onboarding-input-field standard dialog-input onboarding-border-radius-10 q-mt-sm q-mb-md"
		/>
		<q-btn
			@click="deleteUser()"
			flat
			...
			label="Proceed"
			dense
			:class="`onboarding-bg-accent-0 full-width q-py-sm q-mb-s onboarding-border-radius-${
				$q.screen.width < 768 ? '10' : '50'
			}`"
		/>
		<div @click="closeDialog()" class="full-width q-py-sm cursor-pointer text-14">Cancel</div>
	</div>
</template>
    ...
    const deleteUser = () => {
      ...
      ValidatePassword({
        ...
        if (response.status === "success") {
          ...
          $q.notify({
            position: $q.screen.width < 767 ? "top" : "bottom-right",
            classes: "onboarding-error-notif q-px-lg q-pt-none q-pb-none",
            html: true,
